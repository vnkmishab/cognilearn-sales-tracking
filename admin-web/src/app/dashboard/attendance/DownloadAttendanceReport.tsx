...
export default function DownloadAttendanceReport({ attendance }: { attendance: AttendanceRecord[] }) {
  ...
  const handleDownloadAttendance = () => {
    ...
    const hasEmployeeInfo = filteredAttendance.some(record => record.employee);
    ...
    if (hasEmployeeInfo) {
      attendanceHeaders.push('Employee Name', 'Employee Code');
    }
    attendanceHeaders.push('Date', 'Check-In Time', 'Check-Out Time', 'Status', 'Total Hours');
      ...
      {showModal && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 animate-in fade-in duration-200">
          <div className="bg-zinc-900 border border-zinc-800 p-6 rounded-2xl w-full max-w-md shadow-2xl animate-in zoom-in-95 duration-200 text-left">
            <h3 className="text-lg font-semibold text-zinc-100 mb-2">Export Attendance Report</h3>
