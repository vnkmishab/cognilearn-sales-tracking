...
import { fetchMyProfile } from '@/lib/api-server';
...
export default async function EmployeeLayout({
  ...
  try {
    ...
    console.warn('Backend not running yet, using empty profile');
  }
    ...
    <div className="flex h-screen bg-zinc-950 text-zinc-100 font-sans">
      <aside className="w-64 bg-zinc-900 border-r border-zinc-800 flex flex-col shadow-[4px_0_24px_rgba(0,0,0,0.5)] z-10 relative">
        ...
        <div className="px-6 py-4 bg-zinc-950/50 border-b border-zinc-800 text-sm font-medium text-zinc-400">
          Logged in as: <span className="text-zinc-200">{profile ? `${profile.firstName} ${profile.lastName}` : 'Employee'}</span>
          <div className="text-xs text-amber-500/70 mt-1 uppercase tracking-widest">{profile ? profile.employeeCode : ''}</div>
        </div>
        <nav className="flex-1 p-4 space-y-2 text-zinc-400 mt-2">
          <Link href="/employee" className="block px-4 py-3 rounded-lg hover:bg-zinc-800 hover:text-amber-400 hover:shadow-lg hover:shadow-black/20 transition-all font-medium border border-transparent hover:border-zinc-700/50">
            My Dashboard
          </Link>
          <Link href="/employee/notifications" className="block px-4 py-3 rounded-lg hover:bg-zinc-800 hover:text-amber-400 hover:shadow-lg hover:shadow-black/20 transition-all font-medium border border-transparent hover:border-zinc-700/50">
            Notifications
          </Link>
        </nav>
